...
Design invariants (external audit F1 / F2 / F5 / F6):
  ...
  * CURRENT_TREE means PUBLIC CANDIDATE SURFACE, not "whatever is on this
    ...
    --exclude-standard` — and the working-tree contents of those paths are
    read. Git-ignored machine-local artifacts (the local-only recovery
    profile, generated bytecode, caches) are deliberately NOT part of the
    public candidate surface: they are never published, and pulling them in
    would both contradict the local-only profile contract and make scope
    ...
    the reported scope.
  * An input the gate could not read is NOT a pass (F6). A CURRENT_TREE
    candidate above the size cap fails closed as
    UNSCANNED_OVERSIZE_PUBLIC_FILE, with its content withheld. Reads are
    bounded, not streaming: a candidate is read whole up to the cap, and
    anything larger is refused rather than partially inspected. History mode
...
def decode_if_text(data: bytes, scope: Scope | None = None) -> str | None:
    ...
    limit. Anything above MAX_SCAN_BYTES is refused rather than partially
    inspected — in CURRENT_TREE mode the caller turns that refusal into a
    fail-closed violation (F6); in history mode it is recorded as measured
    skipped evidence.
    """
...
def iter_candidate_files(root: Path, scope: Scope | None = None):
    """Yield (relative_path, raw_bytes) for the public candidate surface.

    Reads the *working-tree* contents of each candidate path. Git-ignored
    machine-local files (local recovery profile, generated bytecode, caches)
    are excluded by construction, not by an allowlist.
    """
    ...
    if rels is None:
        ...
        for p in sorted(root.rglob("*")):
            ...
            try:
                ...
                continue
            yield str(p.relative_to(root)).replace("\\", "/"), data
        return
    ...
    for rel in rels:
        p = root / rel
        # Tracked-but-deleted paths, symlinks and unreadable entries are
...
def scan_tree(root: Path, scope: Scope | None = None) -> list[Violation]:
    ...
    F6: a candidate above the read cap is a FAILURE, not a skip. It is
    reported as UNSCANNED_OVERSIZE_PUBLIC_FILE with its content withheld,
    because an input the gate could not inspect cannot be certified clean.
    """
    out: list[Violation] = []
    for rel, data in iter_candidate_files(root, scope):
        if scope:
            scope.blobs_total += 1
        if len(data) > MAX_SCAN_BYTES:
            if scope:
            ...
            out.append(Violation(
                OVERSIZE_SEVERITY, OVERSIZE_RULE, rel, 0,
                f"<NOT SCANNED: {len(data)} bytes exceeds "
