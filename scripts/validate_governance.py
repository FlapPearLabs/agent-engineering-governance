...
# Integration note: the PR#5 scanner-gap fix R1-A (Windows-form host path
# `[a-z]:\users\`) is SUBSUMED, not dropped — vpr.TIER_A_PATTERNS classifies a
# concrete Windows user home as LOCAL_IDENTITY (Tier A, forbidden everywhere),
# which is strictly stronger than the Tier B placement it had in PR#5.
...
def main() -> int:
    ...
    #      - The scanned set is the PUBLIC CANDIDATE SURFACE (F5): tracked +
    #        untracked-non-ignored paths. Git-ignored local recovery profiles
    #        and generated bytecode are out of scope by construction, not by
    #        an allowlist.
    #      - Both publishable CONTENTS of each candidate are scanned (F7): the
    #        INDEX blob, which is what `git commit` actually publishes, and the
    #        working-tree file. A staged secret wiped from disk therefore still
    #        fails here, and a tracked symlink is judged on the target string
    #        Git would publish.
    #      - An oversize candidate the gate could not read fails closed (F6),
    #        with the cap applied to size metadata before any content is
    #        materialized (F7); unscanned is not clean.
    ...
    unscanned = [v.render() for v in tier_findings
                 if v.severity == vpr.OVERSIZE_SEVERITY]
    ...
    check("no-unscanned-oversize-public-candidates", not unscanned,
          f"unscanned={unscanned[:5]}")
