...
| `hooks/bash_preflight_guard.py` | PreToolUse（Bash 命令） | 合同 §7 + R6-F3 + R6.1-1 | receipt 有效性/新鲜度复用 grounding_guard 语义（R6.1-1）：完整新鲜 receipt → 正常 Bash 放行；partial / stale（HEAD 前移）→ BLOCK `GROUNDING_RECEIPT_INVALID` + 具体缺陷；防 receipt-bootstrap 死锁：仅精确放行 canonical 内部 `python <hooks>/codegraph_state.py set-grounding ...`（本 hook 目录 realpath、仅允许 flags，链式/重定向/任意脚本拒绝）；无 receipt → RISK < MEDIUM 放行，否则仅放行**严格只读白名单**（git status/diff/log/show/rev-parse/merge-base/ls-files/branch --show-current、rg/grep/ls/dir/cat/Get-Content；R6.2：`find` 因 -delete/-exec/-execdir/-ok/-fprint* 具备变更与执行能力，不在此列，post-grounding 才恢复）；重定向 / 链接 / 任意脚本 / git 变更 / 文件变更 / 未知命令族 → exit 2 `UNKNOWN_BASH_MUTABILITY`（fail closed，pre-grounding 不存在"看不懂但可能是只读"） |
| `hooks/codegraph_lifecycle.py` | CLI（orchestrator 决策入口） | 合同 §6.7 | 生命周期单一规范决策面：`INIT_ONCE / FULL_INIT_FORBIDDEN / INCREMENTAL_SYNC_ONCE / NO_SYNC / GROUNDING_REQUIRED / BLAST_RADIUS_REQUIRED / BLAST_RADIUS_EXPANSION_REQUIRED / ALLOW_WRITE / MARK_DIRTY / SYNC_FAILED_DEFERRED / CODEGRAPH_REBUILD_REQUIRED（R6-F6 损坏图恢复态）/ CODEGRAPH_UNAVAILABLE（R6.1-2 工具不可用，MODE C，绝不 auto-init / auto-rebuild）/ NO_REPO`；`record-init` WRITE-ONCE（R6-F2，二次 → `GRAPH_INIT_ALREADY_RECORDED`），rebuild 走 `record-rebuild --authority CODEGRAPH_REBUILD_AUTHORIZED`；R6-F3 纵深防御：生命周期边界机械检查 git 生产源码 delta（无 marker / HEAD 不变也强制 sync；MODE A lane = candidate delta 证据）；R6-F5 `MODE_A_BASE_MISMATCH` 拒绝虚假 BASE_ONLY+DELTA_BY_DIFF PASS，R6.1-2 扩展至 review/handoff/stop 且要求 `BASE_SHA == GRAPH_BASE_SHA == CANONICAL_GRAPH_INDEX_SHA`（manual/MODE C 逃逸）；R6-F4 blast radius 三面分离（APPROVED_EDIT_SURFACE 权威 / OBSERVED_DELTA 证据 / IMPACT_SURFACE 感知），`UNAPPROVED_DELTA_DETECTED` + LC-INV8，R6.1-3 未批准 delta 成为运行时门（pre-edit 与 review/handoff/stop 机械 BLOCK）；`verify` 输出 LC-INV1..INV8 不变量（exit 1 = 违规，可接 CI 门禁） |
...
- **R6.1-1 Bash receipt 校验**：`bash_preflight_guard` 不再把 `{BASE_SHA: ...}` 形状当作充分 grounding——复用 grounding_guard 的 receipt 有效性/新鲜度语义（partial / stale → `GROUNDING_RECEIPT_INVALID` BLOCK；完整新鲜 → 正常 Bash）。防 receipt-bootstrap 死锁：仅精确放行 canonical 内部 `python <hooks>/codegraph_state.py set-grounding ...`（本 hook 目录 realpath + 仅允许 flags，链式/重定向/任意脚本拒绝）。测试：R61-F1（×2，含端到端 bootstrap）。
- **R6.1-2 Mode A 一致性覆盖 review + Mode C 逃逸 + 工具不可用分类**：graph-backed Mode A 在 pre-edit / review / grounding 校验强制 `BASE_SHA == GRAPH_BASE_SHA == CANONICAL_GRAPH_INDEX_SHA`——长寿命 lane 的 review 绝不返回正常 `BASE_ONLY + DELTA_BY_DIFF PASS`；`GRAPH_MODE=manual`（MODE C）真实逃逸。CLI/工具不可用（未安装 / 探测进程错误）→ 第 14 个封闭枚举 `CODEGRAPH_UNAVAILABLE`（MODE C，绝不 auto-init / auto-rebuild）；健康工具报告损坏 index → `CODEGRAPH_REBUILD_REQUIRED`（R6-F6 不变）。测试：R61-F2（×2）。
- **R6.1-3 未批准观测 delta 运行时门**：`OBSERVED_DELTA ⊄ APPROVED_EDIT_SURFACE` → pre-edit 与 review/handoff/stop 双路径机械 BLOCK（review/handoff/stop 门仅要求存在带 unapproved delta 的 blast_radius 记录，不要求 canonical graph 已 init），直至 delta 移除或显式扩张批准面。LC-INV8 保持纵深防御，不再是唯一强制路径。测试：R61-F3。
