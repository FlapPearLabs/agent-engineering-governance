...
> **MACHINE-SPECIFIC ALLOWED** —— 本文件是 RULES R2 第二层的 designated deployment 产物：其中环境事实段可含宿主路径/端口（私有仓、用途 = 新机 bootstrap）；**凭据/secret/local OS identity 仍然绝对禁止**（R2 第一层，designated 不豁免）。
