...
> Canonical owner: AGENTS.md §6/§9。本文件是字段表、判据与模板。

## 1. 评审分级（L0/L1/L2）

| 层 | 承担者 | 核验内容 | 触发 |
|---|---|---|---|
| L0 MACHINE | harness / 脚本 / agent 机械执行 | exact SHA、base SHA、diff 范围、测试执行与结果、回归、ancestry、禁改文件、静态检查、secret/路径扫描、机械反例清单、图 base SHA 记录 | 每票（LOW 票 L0+轻 L1 即可闭合） |
| L1 NORMAL | 低成本 fresh context 独立评审 | 合同满足、缝与所有权、scope、失败语义、缺失假设、缺失高价值反例（≥2 个非复制 worker 的新反例） | MEDIUM 全部；LOW 抽样 |
| L2 STRONG/EXTERNAL | 强模型/外部独立评审（不同模型族优先） | 架构、安全、状态/并发、身份/provenance、评审分歧仲裁、governance/Spec 变更、里程碑、高爆炸半径 | HIGH/CRITICAL；分歧；milestone |

独立性语义：fresh context + 独立 grounding + 独立反例；`INDEPENDENT_CODEGRAPH_GROUNDING != INDEPENDENT_FULL_REINDEX`。
...
经济学 = 期望风险降低 vs（复杂度 + 回归风险）。加固若可能拒绝当前合法输出且权威不明确 → 不自动修（R9）。
...
- `NORMAL_REVIEWER_DRIVEN_REPAIR_BUDGET = 2`：初始候选 → fresh review → 修复轮 1（仅高价值 blocker）→ fresh review → 修复轮 2（仅高价值 blocker）→ `AUTO_REPAIR_AUTHORITY = EXHAUSTED`。
...
- 终局问题："这个 exact 候选是否对现实可达状态正确实现其拥有的合同、并在真实信任边界安全失败、无已知高价值 blocker？"——不是"还能不能编出更多畸形态？"
...
NEW_CODEGRAPH_FINDINGS =      # 关系/依赖/重复合同/隐藏状态路径/意外调用方/所有权意外；无则 NONE
...
# GATES                      （常规行压缩；任何非 PASS 状态必须展开证据块，见 git-ci-integration）
EXACT_SHA / CODEGRAPH / TDD / TESTS / CODE_REVIEW / THIRD_PARTY / PR_CI / POST_CI_REVIEW

# DECISION
UNRESOLVED_P0 / UNRESOLVED_P1 / READY_FOR_EXTERNAL_REVIEW
```

已知事实不重述（规则存在性、常规 TDD 序列、thin orchestrator 等）；仅在被违反、异常影响、或明确索要审计细节时展开。`PR_CI_COMPRESSION_ALLOWED = PASS_ONLY`。
